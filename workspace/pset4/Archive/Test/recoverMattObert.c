 ...
 * 
 * the fault here is that, we are printing every 512 byte into a file each..
 * new discover, &store[1] means the address for store[1](where store[1] lives in memory, which can be '0x63d'). 
 */
   ...
   FILE* inptr = fopen("card.raw", "r");
    ...
    int cardSize = ftell(inptr);
        ...
        for(int h = 0, noOfBlocks = cardSize/512; h < noOfBlocks; h++)
        {
            ...
            //****this shows that the code is just reading the same bytes over and over again.. Is that true..?
            ...
            if(jpgCheck(store, jpgSig) && foundJpg)
            {
                //fileNameCounter ++
                fileNameCounter++;
                //foundJpg = false

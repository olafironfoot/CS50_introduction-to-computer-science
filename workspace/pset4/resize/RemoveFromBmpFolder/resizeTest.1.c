...
//fields to modify for bf & bi? (suggested by Pset) - bi.Size, bi.Width, bi.Height
    ...
    bf.bfSize = bfStructSize + bi.biSize + bi.biSizeImage + paddingInptr;
    ...
    for (int i = 0, oldHeight = abs(oldBiHeight); i < oldHeight; i++)
        {
            ...
            for(int m = 0; m < n; m++)
            {
                //move the cur before reading file...find  position of CUR (within the line/row, SEEK_SET goes to the front of the row again)
